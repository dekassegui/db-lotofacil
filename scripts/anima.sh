...
if [[ ! $( which ffmpeg ) ]]; then
  ...
  . scripts/hack.sh   # agrega quadros de transição suave
  ...
  ffmpeg -f concat -i video/roteiro.txt -i $audio -vf 'scale=800:600' \
    -c:v libx264 -profile:v baseline -preset $speed -tune $tune -crf $quality \
    -pix_fmt yuv420p -c:a aac -b:a 96k -y video/fun.mp4
